...
async def test_stop_kills_shell_pids(tmp_config):
    """Verify stop message kills running shell PIDs."""
    ...
    with patch("os.kill", side_effect=mock_kill):
        ...
        # Verify all PIDs were killed with SIGKILL (signal 9)
        assert len(killed_pids) == 2
        assert (12345, 9) in killed_pids
        assert (12346, 9) in killed_pids
