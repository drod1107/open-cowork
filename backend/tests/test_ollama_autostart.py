...
- Start Ollama process if conditions are met

Why these tests FAIL:
- `run()` function doesn't have Ollama auto-start logic yet
- Dev team needs to implement:
  1. Check for Ollama binary with `shutil.which("ollama")`
  2. Check if port 11434 is free with `socket.connect_ex(("localhost", 11434))`
  3. Start Ollama with `subprocess.Popen(["ollama", "serve"])`
"""
...
async def test_run_function_has_ollama_auto_start():
    ...
    Why it FAILS:
    - `run()` doesn't have the auto-start logic yet
    - Dev needs to add: binary check, port check, subprocess start
    """
...
async def test_run_function_starts_ollama_process():
    ...
    Why it FAILS:
    - `run()` doesn't start Ollama process
    - Dev needs to add: `subprocess.Popen(['ollama', 'serve'])` or similar
    """
    from backend import main
